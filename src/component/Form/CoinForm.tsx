...
const getClipboardText = (dividedCoins: DividedCoins) =>
  'PP: ' +
  dividedCoins.dividedCoins[0].platinum +
  ',' +
  'GP: ' +
  dividedCoins.dividedCoins[0].gold +
  ',' +
  'EP: ' +
  dividedCoins.dividedCoins[0].electrum +
  ',' +
  'SP: ' +
  dividedCoins.dividedCoins[0].silver +
  ',' +
  'CP: ' +
  dividedCoins.dividedCoins[0].copper
    ...
    <>
        ...
        {outputDividedCoins && (
          <div className={styles.Result}>
            ...
            <span className={styles.text}>
              PP: {outputDividedCoins?.platinum}, GP: {outputDividedCoins?.gold}, EP:{' '}
              {outputDividedCoins?.electrum}, SP: {outputDividedCoins?.silver}, CP:{' '}
              {outputDividedCoins?.copper}
            </span>
          </div>
        )}
        {outputRemainingCoins && (
          <div className={styles.Result}>
            <h2 className={styles.subtitle}>Remainder:</h2>
            <span className={styles.text}>
              PP: {outputRemainingCoins?.platinum}, GP: {outputRemainingCoins?.gold}, EP:{' '}
              {outputRemainingCoins?.electrum}, SP: {outputRemainingCoins?.silver}, CP:{' '}
              {outputRemainingCoins?.copper}
            </span>
          </div>
        )}
      </output>
      ...
      {outputDividedCoins && (
        <div className={styles.outputAction}>
          <Button onClick={() => copyHandler(dividedCoins)}>Copy to clipboard</Button>
        </div>
      )}
    </>
  )
